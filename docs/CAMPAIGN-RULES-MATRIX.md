...
| Private objectives | Configure, Secret, Calculate, Audit | Catalog at setup; at launch assign one secret objective from each player, faction, and ally-group pool. Traitor-kind entries are not seeded. Skip empty pools. Unique draws first, then reshuffled duplicates so every holder in a non-empty pool receives an assignment, except a player never holds the same catalog type twice (player-held and Traitor-held share that uniqueness). Manual claims need manager approval. Automatic criteria score from map facts and become public when completed. Unclaimed manual objectives do not score when the campaign ends. The viewer's own assignments are listed; other holders' claimed or revealed objectives appear in a collapsed subpanel. Unclaimed text stays secret until reveal or campaign completion. |
