...
| Force statuses                       | Calculate, Display, Audit     | Track one named status per force (no named status is Normal and is not shown as a status chip). Unique priorities 0-999 (0 highest) decide which status remains when more than one would apply. A status may cancel another to no status. Enable and clear each list one or more conditions (a trigger plus 1-10 consecutive matches, default 1; at least one of each; unique triggers per list). Any listed condition can apply: battle triggers count consecutive battles, water and Hold triggers count consecutive action phases. Named Diseased uses consecutive water-feature actions, water battles, contagion, rejoin, and plague-bearing wins; Hold at a Capital City, City, Supply Depot, or Town clears it to no status (or that many consecutive settlement Holds when that clear condition's count is greater than 1). Missions and item results may list ordered status-change conditions. Managers and administrators may assign any catalog status. Effect text is display-only. Status changes are logged with source. |
