...
| Campaign presets                     | Configure                     | Built-in Hunt in Estalia catalog plus administrator-saved named presets (from Edit campaign or Edit map) that include map image and overlay graph, remapped by catalog name on apply. Duplicate names after trimming whitespace overwrite. |
...
| Points per territory                 | Calculate                     | When configured above 0, each currently owned territory awards that many public-objective campaign points. |
