...
| Split-force supply | Configure, Calculate | Each split force claims the map supply of the chain it can reach minus the split penalty (Hunt default raw 1, or a configured 0–100 percent), minimum 1 each, plus the full round free-supply bonus. Temporary supply remains a shared player pool. |
