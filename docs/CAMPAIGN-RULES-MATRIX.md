...
| Backstab                             | Enforce, Calculate, Audit     | Only when sharing a territory with an allied force or occupying that ally's empty land. Break the alliance. Empty former-ally land: claim and auto-pillage (never auto-destroy). Shared land: create a battle with no auto-pillage. |
...
| Private objectives                   | Configure, Secret, Calculate, Audit | Catalog at setup; at launch assign one secret objective from each holder-kind pool (players, factions, ally groups). Skip empty pools. Unique draws first, then reshuffled duplicates so every holder in a non-empty pool receives an independent assignment. Manual claims need manager approval. Automatic criteria score from map facts and become public when completed. Unclaimed manual objectives do not score when the campaign ends. The viewer's own assignments are listed; other holders' claimed or revealed objectives appear in a collapsed subpanel. Unclaimed text stays secret until reveal or campaign completion. |
