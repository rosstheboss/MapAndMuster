...
| Move | Enforce, Calculate | Validate controller, origin, destination, adjacency, spawn restrictions, force movement speed (default 1; Bretonnia preset 2; item bonuses; CalledByTheRelic +1 while a revealed item exists and no force of that faction holds an item), and named via when several routes exist. Only the landing territory is claimed. Invalid Move becomes Hold. |
