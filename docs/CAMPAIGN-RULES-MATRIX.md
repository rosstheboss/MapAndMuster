...
| Private objectives                   | Configure, Secret, Calculate, Audit | Catalog at setup; at launch assign one secret objective from each holder-kind pool (players, factions, ally groups). Skip empty pools. Unique draws first, then reshuffled duplicates so every holder in a non-empty pool receives an independent assignment. Manual claims need manager approval. Automatic criteria score from map facts and become public when completed. Unclaimed counts are public; text stays secret until reveal or campaign completion. |
