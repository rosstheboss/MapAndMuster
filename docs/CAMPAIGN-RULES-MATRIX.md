...
| Retreat | Enforce, Calculate, Audit | Validate eligible destinations (open-neutral, owned, or allied; Art of War any non-enemy-spawn), resolve collisions by strongest (campaign points, territories, structures, supply including temporary, then recorded random), and assign spawn when no order is given or no eligible destination remains. |
...
| Battle result reporting | Configure, Enforce, Audit | Players report both sides. Agree, dispute, staff confirm, or deadline-commit. One timely submission is authoritative. Neither submission is a no-contest forced retreat (skip Shaken, no spoils). True BP ties force both to retreat without a loss. Campaign managers configure reusable standard result questions and attach them to missions, with optional per-mission point overrides; unique per-mission questions remain. |
