...
public sealed class CampaignEndpointTests
{
    ...
    public CampaignEndpointTests(MapAndMusterApiFactory factory)
    {
        _factory = factory;
    }
    ...
    [Fact]
    public void ImportPresetPackageAllowsABodyLargerThanTheDefaultKestrelLimit()
    {
        var endpoint = _factory.Services.GetRequiredService<EndpointDataSource>().Endpoints
            ...
            .Single(item =>
                string.Equals(
                    ...
                    "ImportCampaignPresetPackage",
                    StringComparison.Ordinal));
        var requestSize = endpoint.Metadata.GetMetadata<IRequestSizeLimitMetadata>();
        Assert.NotNull(requestSize);
        Assert.Equal(ImportCampaignPresetHandler.MaxPackageBytes, requestSize.MaxRequestBodySize);

        var form = endpoint.Metadata.GetMetadata<IFormOptionsMetadata>();
        Assert.NotNull(form);
        Assert.Equal(ImportCampaignPresetHandler.MaxPackageBytes, form.MultipartBodyLengthLimit);
    }

    [Fact]
    public async Task NonAdministratorsCannotDownloadOrUploadPresetPackages()
    {
        using var client = _factory.CreateClient();
        var username = UniqueName("prepkg");
        await RegisterConfirmAndLoginAsync(client, $"{username}@example.test", username);

        using var createdResponse = await client.PostAsJsonAsync("/api/campaigns", ValidCampaignBody("Package Border"));
        var created = await createdResponse.Content.ReadFromJsonAsync<CampaignDetailResponse>(JsonOptions);
        Assert.NotNull(created);

        using var download = await client.GetAsync($"/api/campaigns/{created.Id}/preset-package");
        Assert.Equal(HttpStatusCode.Forbidden, download.StatusCode);
        var downloadBody = await download.Content.ReadFromJsonAsync<ErrorResponse>(JsonOptions);
        Assert.Equal("campaign.forbidden", downloadBody?.Code);

        using var uploadContent = new MultipartFormDataContent();
        uploadContent.Add(new ByteArrayContent([1, 2, 3]), "package", "border.mapandmuster-preset");
        using var upload = await client.PostAsync("/api/campaign-presets/package", uploadContent);
        Assert.Equal(HttpStatusCode.Forbidden, upload.StatusCode);
        var uploadBody = await upload.Content.ReadFromJsonAsync<ErrorResponse>(JsonOptions);
        Assert.Equal("campaign.forbidden", uploadBody?.Code);
    }

    [Fact]
    public async Task AdministratorCanDownloadAndUploadACampaignPresetPackage()
    {
        using var client = _factory.CreateClient();
        var username = UniqueName("pkgadm");
        var email = $"{username}@example.test";
        await RegisterConfirmAndLoginAsync(client, email, username);

        using var createdResponse = await client.PostAsJsonAsync("/api/campaigns", ValidCampaignBody("Portable Border"));
        var created = await createdResponse.Content.ReadFromJsonAsync<CampaignDetailResponse>(JsonOptions);
        Assert.NotNull(created);
        ...
        Assert.Equal("application/zip", exportedResponse.Content.Headers.ContentType?.MediaType);
        Assert.Contains(
            "portable-border-preset.mapandmuster-preset",
            exportedResponse.Content.Headers.ContentDisposition?.FileNameStar
                ?? exportedResponse.Content.Headers.ContentDisposition?.FileName
                ?? string.Empty,
        ...
        using (var zip = new ZipArchive(new MemoryStream(packageBytes), ZipArchiveMode.Read))
        {
            ...
            Assert.Contains("catalog.json", names);
            Assert.Contains("settings.json", names);
            Assert.Contains("overlay.json", names);
            Assert.Contains("overlay.svg", names);
            Assert.Contains("map.png", names);
            using var overlayJson = zip.GetEntry("overlay.json")!.Open();
            using var overlayReader = new StreamReader(overlayJson);
            Assert.Contains("Westmarch", overlayReader.ReadToEnd(), StringComparison.Ordinal);
        }
        ...
        importContent.Add(package, "package", "portable-border-preset.mapandmuster-preset");
        ...
        using var targetResponse = await client.PostAsJsonAsync("/api/campaigns", ValidCampaignBody("Empty Portable"));
        var target = await targetResponse.Content.ReadFromJsonAsync<CampaignDetailResponse>(JsonOptions);
        Assert.NotNull(target);
        ...
        var graph = await client.GetFromJsonAsync<MapGraphResponse>($"/api/campaigns/{target.Id}/map/graph", JsonOptions);
        Assert.NotNull(graph);
        Assert.Equal("Westmarch", graph.Territories.Single().Name);
        Assert.Equal(
            target.TerrainTypes.Single(type => type.Name == "Plains").Id,
            graph.Territories.Single().TerrainTypeId);

        using var importedMap = await client.GetAsync($"/api/campaigns/{target.Id}/map");
        Assert.Equal(HttpStatusCode.OK, importedMap.StatusCode);
        Assert.Equal("image/png", importedMap.Content.Headers.ContentType?.MediaType);

        using var invalidContent = new MultipartFormDataContent();
        invalidContent.Add(new ByteArrayContent([1, 2, 3]), "package", "broken.mapandmuster-preset");
        using var invalid = await client.PostAsync("/api/campaign-presets/package", invalidContent);
        Assert.Equal(HttpStatusCode.BadRequest, invalid.StatusCode);
        var invalidBody = await invalid.Content.ReadFromJsonAsync<ErrorResponse>(JsonOptions);
        Assert.Equal("campaign.preset.package.invalid", invalidBody?.Code);
    }
