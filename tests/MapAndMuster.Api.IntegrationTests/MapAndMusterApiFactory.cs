...
public sealed class MapAndMusterApiFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    private readonly PostgreSqlContainer _postgres = new PostgreSqlBuilder("postgres:17-alpine")
        .WithDatabase("mapandmuster_tests")
        .WithUsername("mapandmuster")
        .WithPassword("mapandmuster")
        .Build();

    private readonly string _storagePath = Path.Combine(Path.GetTempPath(), "mapandmuster-test-storage", Guid.NewGuid().ToString("N"));
...
public sealed class ApiTestGroup : ICollectionFixture<MapAndMusterApiFactory>
{
}
