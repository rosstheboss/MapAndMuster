...
describe('CampaignsPage', () => {
  ...
  it('groups campaigns and keeps them collapsed until expanded', async () => {
    ...
    const names = [...compiled.querySelectorAll('button.section-toggle')].map((button) => button.textContent.trim());
    expect(names).toEqual(['Sooner War', 'Current War', 'Border War', 'Later War', 'Recent War', 'Old War']);
    const upcoming = [...compiled.querySelectorAll<HTMLButtonElement>('button.section-toggle')].find((button) =>
      button.textContent.includes('Border War'),
    );
    expect(upcoming?.getAttribute('aria-expanded')).toBe('false');
    const hiddenBody = upcoming?.nextElementSibling as HTMLElement | null;
    expect(hiddenBody?.hidden).toBe(true);
