...
public static class IdentityHttp
{
    ...
    private static IResult Problem(string errorCode, string message, IReadOnlyList<MapAndMuster.Domain.Common.DomainError> errors)
    {
    ...
    private static FieldErrorResponse[]? ToFieldErrors(IReadOnlyList<MapAndMuster.Domain.Common.DomainError> errors)
    {
        if (errors.Count == 0)
        {
            return null;
